...
class VpnLogic:
    ...
    def connect(self, server_ip, username, password, protocol):
        ...
        # This command is a basic template and requires a configured server.
        proto_map = {"Авто": "udp", "OpenVPN": "udp"} # Defaulting to UDP
        command = [
            self.openvpn_path,
            "--client",
            "--dev", "tun",
            "--proto", proto_map.get(protocol, "udp"),
            "--remote", server_ip,
            "--auth-user-pass", auth_file_path,
            "--verb", "3",
            "--pull",
            # NOTE: This requires the user to manually place ca.crt in the config directory.
...
class App(Tk):
    def __init__(self):
        ...
        self.protocol.set("Авто") # Set default protocol
    ...
    def _create_widgets(self):
        ...
        protocol_menu = ttk.Combobox(conn_frame, textvariable=self.protocol, values=["Авто", "OpenVPN"], state="readonly")
        ...
        Label(split_tunnel_frame, text="Split Tunneling: Исключения", bg=Colors["BG"], fg=Colors["FG"], font=("Helvetica", 12, "bold")).pack(anchor='w')
    ...
    def _connect(self):
        ...
        if not all([ip, user, pwd, proto]):
            ...
            return
        self._run_in_thread(self.vpn.connect, ip, user, pwd, proto)

    def _disconnect(self):
        self._run_in_thread(self.vpn.disconnect)

    def _add_exception(self):
        selected_process = self.active_processes.get()
        if selected_process and selected_process not in self.exception_listbox.get(0, "end"):
            self.exception_listbox.insert("end", selected_process)
