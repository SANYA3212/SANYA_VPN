...
1.  **Download the Repository:** Download and extract the project files to a folder on your Windows PC.
2.  **Launch the Client:** Double-click the `SANYA-VPN.bat` file.
3.  **Automatic Installation:** On the first run, the application will check if its VPN engine is installed. If not, it will prompt for **administrator privileges** to download and install it into a local `vpn_engine` folder. Please accept the prompt. This is a one-time setup.
4.  **Configure the Client:**
    ...
    *   **Password:** Enter `rootsanyapi1` (the default password).
...
*   **The client fails to download or install the VPN engine automatically.**
    *   **Solution:** Ensure your internet connection is active and that you have permissions to write to the application's folder. You can also try running `SANYA-VPN.bat` as an administrator.
